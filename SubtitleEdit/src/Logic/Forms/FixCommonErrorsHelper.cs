namespace Nikse.SubtitleEdit.Logic.Forms
{
    ...
    public static class FixCommonErrorsHelper
    {
        ...
        public static string FixDialogsOnOneLine(string text, string language)
        {
            ...
            {
                string[] noTagLines = HtmlUtil.RemoveHtmlTags(text.Replace(" - ", Environment.NewLine), true).SplitToLines();
                if (noTagLines.Length == 2)
                {
                    string part0 = noTagLines[0];
                    string part1 = noTagLines[1];
                    if (part0.Length > 1 && "-—!?.\")]".Contains(part0[part0.Length - 1]) && part1.Length > 1 && ("'" + Utilities.UppercaseLetters).Contains(part1[0]))
                    {
            ...
            string[] stringArray = { ". -", "! -", "? -", "— -", "-- -", ") -", "] -", "> -" };
            int idx = text.IndexOfAny(stringArray, StringComparison.Ordinal);
            if (idx >= 0)
            {
                if (Utilities.GetNumberOfLines(text) == 2)
                {
                    ...
                    string[] arrTemp = temp.SplitToLines();
                    if (arrTemp.Length == 2 && arr.Length == 2)
                    {
                        string secLine = HtmlUtil.RemoveHtmlTags(arr[1]).TrimStart();
                        string secLineTemp = HtmlUtil.RemoveHtmlTags(arrTemp[1]).TrimStart();
        ...
        public static string RemoveSpacesBeginLineAfterEllipses(string line)
        {
            ...
            if (line.Length > 6 && line.LineStartsWithHtmlTag(true))
            {
                // <i>... foobar
            ...
            if (line.LineStartsWithHtmlTag(false, true))
            {
                // <font color="#000000"> and <font>
                int closeIdx = line.IndexOf('>', 5);
                if (closeIdx >= 5 && line.Length > closeIdx + 5)
                {
                    string fontTag = line.Substring(0, closeIdx + 1).TrimStart();
        ...
        internal static string FixShortLines(string text)
        {
            ...
            if (s.Contains(Environment.NewLine) && s.Replace(Environment.NewLine, " ").Replace("  ", " ").Length < Configuration.Settings.Tools.MergeLinesShorterThan)
            {
                s = s.TrimEnd().TrimEnd('.', '?', '!', ':', ';');
                s = s.TrimStart('-');
                if (!s.Contains(new[] { '.', '?', '!', ':', ';', '-', '♪', '♫' }) && !(s.StartsWith('[') && s.Contains("]" + Environment.NewLine, StringComparison.Ordinal)) && !(s.StartsWith('(') && s.Contains(")" + Environment.NewLine, StringComparison.Ordinal)) && s != s.ToUpper())
                {
