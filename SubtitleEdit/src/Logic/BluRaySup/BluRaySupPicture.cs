namespace Nikse.SubtitleEdit.Logic.BluRaySup
{
    ...
    public class BluRaySupPicture
    {
        ...
        public int StartTimeForWrite
        {
            get
            {
                return (int)((this.StartTime - 45) * 90.0);
            }
        }
        ...
        public int EndTimeForWrite
        {
            get
            {
                return (int)((this.EndTime - 45) * 90.0);
            }
        }
        ...
        public static byte[] CreateSupFrame(BluRaySupPicture pic, Bitmap bmp, double fps, int bottomMargin, int leftOrRightMargin, ContentAlignment alignment)
        {
            ...
            Dictionary<Color, int> colorPalette = GetBitmapPalette(bm);
            BluRaySupPalette pal = new BluRaySupPalette(colorPalette.Count);
            int k = 0;
            foreach (KeyValuePair<Color, int> kvp in colorPalette)
            {
            ...
            byte[] packetHeader = { 0x50, 0x47, // 0:  "PG"
            ...
            byte[] headerPcsStart = { 0x00, 0x00, 0x00, 0x00, // 0: video_width, video_height
            ...
            byte[] headerPcsEnd = { 0x00, 0x00, 0x00, 0x00, // 0: video_width, video_height
                                    0x10, // 4: hi nibble: frame_rate (0x10=24p), lo nibble: reserved
                                    0x00, 0x00, // 5: composition_number (increased by start and end header)
            ...
            byte[] headerWds = { 0x01, // 0 : number of windows (currently assumed 1, 0..2 is legal)
                                 0x00, // 1 : window id (0..1)
                                 0x00, 0x00, 0x00, 0x00, // 2 : x-ofs, y-ofs
                                 0x00, 0x00, 0x00, 0x00 // 6 : width, height
                               };
            byte[] headerOdsFirst = { 0x00, 0x00, // 0: object_id
            ...
            byte[] headerOdsNext = { 0x00, 0x00, // 0: object_id
                                     0x00, // 2: object_version_number
        ...
        private static int GetFpsId(double fps)
        {
            ...
            if (Math.Abs(fps - Core.FpsPal) < 0.01)
            {
                // 25
                return 0x30;
            }

            if (Math.Abs(fps - Core.FpsNtsc) < 0.01)
            {
                // 29.97
                return 0x40;
            }

            if (Math.Abs(fps - Core.FpsPalI) < 0.01)
            {
                // 50
                return 0x60;
            }

            if (Math.Abs(fps - Core.FpsNtscI) < 0.1)
            {
                // 59.94
                return 0x70;
            }
