    ...
    internal static class FileUtil
    {
        ...
        public static bool IsSrr(string fileName)
        {
            using (var fs = new FileStream(fileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                ...
                return buffer[0] == 0x69 && buffer[1] == 0x69 && buffer[2] == 0x69;
            }
        }
        ...
        public static bool IsVobSub(string fileName)
        {
            using (var fs = new FileStream(fileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                var buffer = new byte[4];
                fs.Read(buffer, 0, buffer.Length);
                return VobSubParser.IsMpeg2PackHeader(buffer) || VobSubParser.IsPrivateStream1(buffer, 0);
            }
        }
        ...
        /// </param>
        /// <returns>
        /// true if file is an MXF file, otherwise false
        /// </returns>
        public static bool IsMaterialExchangeFormat(string fileName)
        {
            using (var fs = new FileStream(fileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                ...
                for (int i = 0; i < count - 11; i++)
                {
                    ...
                    if (buffer[i + 00] == 0x06 && buffer[i + 01] == 0x0E && buffer[i + 02] == 0x2B && buffer[i + 03] == 0x34 && buffer[i + 04] == 0x02 && buffer[i + 05] == 0x05 && buffer[i + 06] == 0x01 && buffer[i + 07] == 0x01 && buffer[i + 08] == 0x0D && buffer[i + 09] == 0x01 && buffer[i + 10] == 0x02)
                    {
                        return true;
                    }
                }
        ...
        /// <summary>
        /// The is subtitle file all binary zeroes.
        /// </summary>
        /// <param name="fileName">
        /// The file name.
        /// </param>
