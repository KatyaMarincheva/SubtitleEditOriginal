    ...
    public class RemoveTextForHI
    {
            ...
            for (int i = 6; i < newText.Length; i++)
            {
                ...
                if (s.Length > 2 && (s.StartsWith('.') || s.StartsWith('!') || s.StartsWith('?')))
                {
                    ...
                    if (pre.Length > 0)
                    {
                        ...
                        if (this.HasHearImpariedTagsAtStart(s))
                        {
                            s = this.RemoveStartEndTags(s);
        ...
        public string RemoveColon(string text)
        {
            ...
            foreach (string line in lines)
            {
                ...
                if (indexOfColon > 0 && this.IsNotInsideBrackets(text, indexOfColon))
                {
                    string pre = line.Substring(0, indexOfColon);
                    string noTagPre = HtmlUtil.RemoveHtmlTags(pre, true);
                    if (this.Settings.RemoveTextBeforeColonOnlyUppercase && noTagPre != noTagPre.ToUpper())
                    {
                    ...
                    else
                    {
                        ...
                        if (count == 1 && newText.Length > 1 && removedInFirstLine && Utilities.CountTagInText(line, ':') == 1 && ".?!".IndexOf(newText[newText.Length - 1]) < 0 && newText.LineEndsWithHtmlTag(true) && line != line.ToUpper())
                        {
                        ...
                        else if (count == 1 && newText.Length > 1 && indexOfColon > 15 && line.Substring(0, indexOfColon).Contains(' ') && Utilities.CountTagInText(line, ':') == 1 && ".?!".IndexOf(newText[newText.Length - 1]) < 0 && newText.LineEndsWithHtmlTag(true) && line != line.ToUpper())
                        {
                            if (pre.Contains("<i>") && line.Contains("</i>"))
                            {
                                newText = newText + Environment.NewLine + "<i>" + line;
                        ...
                        else if (Utilities.CountTagInText(line, ':') == 1)
                        {
                            ...
                            if (!this.DoRemove(pre))
                            {
                                remove = false;
                            }

                            if (remove && this.Settings.ColonSeparateLine)
                            {
                            ...
                            if (remove)
                            {
                                string content = line.Substring(indexOfColon + 1).Trim();
                                if (content.Length > 0)
                                {
                                ...
                                if (!this.IsHIDescription(st.StrippedText))
                                {
                                    noOfNames++;
                                }
                            }
                        ...
                        else
                        {
                            ...
                            for (int k = 0; k < 2; k++)
                            {
                                if (s2.Contains(':'))
                                {
                                    ...
                                    if (!this.Settings.RemoveTextBeforeColonOnlyUppercase || start == start.ToUpper())
                                    {
            ...
            if (noOfNames > 0 && Utilities.GetNumberOfLines(newText) == 2)
            {
                ...
                string[] arr = newText.SplitToLines();
                if (arr.Length == 2 && arr[0].Length > 1 && arr[1].Length > 1)
                {
                    ...
                    // line continuation?
                    if (arr0.Length > 0 && arr1.Length > 1 && (Utilities.LowercaseLetters + ",").Contains(arr0.Substring(arr0.Length - 1), StringComparison.Ordinal) && Utilities.LowercaseLetters.Contains(arr1[0]))
                    {
                    ...
                    if (arr0.Length > 0 && arr1.Length > 1 && !(tempArr0QuoteTrimmed.EndsWith('.') || tempArr0QuoteTrimmed.EndsWith('!') || tempArr0QuoteTrimmed.EndsWith('?') || tempArr0QuoteTrimmed.EndsWith("</i>", StringComparison.Ordinal)) && !new StripableText(arr[1]).Pre.Contains('-'))
                    {
                        insertDash = false;
                    }
                ...
                if (insertDash)
                {
                    ...
                    {
                        StripableText st = new StripableText(newText, string.Empty, string.Empty);
                    ...
                    {
                        StripableText st = new StripableText(second, string.Empty, string.Empty);
        ...
        public string RemoveTextFromHearImpaired(string text)
        {
            ...
            {
                foreach (string removeIfTextContain in this.Settings.RemoveIfTextContains)
                {
            ...
            string oldText = text;
            text = this.RemoveColon(text);
            string pre = " >-\"'‘`´♪¿¡.…—";
            string post = " -\"'`´♪.!?:…—";
            if (this.Settings.RemoveTextBetweenCustomTags)
            {
                pre = pre.Replace(this.Settings.CustomStart, string.Empty);
                post = post.Replace(this.Settings.CustomEnd, string.Empty);
            }

            StripableText st = new StripableText(text, pre, post);
            StringBuilder sb = new StringBuilder();
            string[] parts = st.StrippedText.Trim().SplitToLines();
            int lineNumber = 0;
            ...
            foreach (string s in parts)
            {
                ...
                if (!this.StartAndEndsWithHearImpariedTags(tempStrippedtext))
                {
                ...
                {
                    if (!this.IsHIDescription(stSub.StrippedText))
                    {
                        noOfNamesRemoved++;
                        if (lineNumber > 0)
                        {
            ...
            text = this.RemoveEmptyFontTag(text);
            text = text.Replace("  ", " ").Trim();
            text = this.RemoveColon(text);
            text = this.RemoveLineIfAllUppercase(text);
            text = this.RemoveHearImpairedtagsInsideLine(text);
            if (this.Settings.RemoveInterjections)
            {
                text = this.RemoveInterjections(text);
            }

            st = new StripableText(text, " >-\"'‘`´♪¿¡.…—", " -\"'`´♪.!?:…—");
            text = st.StrippedText;
            if (this.StartAndEndsWithHearImpariedTags(text))
            {
                text = this.RemoveStartEndTags(text);
            }

            text = this.RemoveHearImpairedTags(text);
            ...
            if (oldText.TrimStart().StartsWith("- ", StringComparison.Ordinal) && text != null && !text.Contains(Environment.NewLine) && (oldText.Contains(Environment.NewLine + "- ", StringComparison.Ordinal) || oldText.Contains(Environment.NewLine + " - ", StringComparison.Ordinal) || oldText.Contains(Environment.NewLine + "<i>- ", StringComparison.Ordinal) || oldText.Contains(Environment.NewLine + "<i> - ", StringComparison.Ordinal)))
            {
                text = text.TrimStart().TrimStart('-').TrimStart();
            }

            if (oldText.TrimStart().StartsWith("-", StringComparison.Ordinal) && !oldText.TrimStart().StartsWith("--", StringComparison.Ordinal) && text != null && !text.Contains(Environment.NewLine) && (oldText.Contains(Environment.NewLine + "-", StringComparison.Ordinal) && !oldText.Contains(Environment.NewLine + "--", StringComparison.Ordinal) || oldText.Contains(Environment.NewLine + " - ", StringComparison.Ordinal) || oldText.Contains(Environment.NewLine + "<i>- ", StringComparison.Ordinal) || oldText.Contains(Environment.NewLine + "<i> - ", StringComparison.Ordinal)))
            {
                text = text.TrimStart().TrimStart('-').TrimStart();
            }

            if (oldText.TrimStart().StartsWith("<i>- ", StringComparison.Ordinal) && text != null && text.StartsWith("<i>- ", StringComparison.Ordinal) && !text.Contains(Environment.NewLine, StringComparison.Ordinal) && (oldText.Contains(Environment.NewLine + "- ", StringComparison.Ordinal) || oldText.Contains(Environment.NewLine + " - ", StringComparison.Ordinal) || oldText.Contains(Environment.NewLine + "<i>- ", StringComparison.Ordinal) || oldText.Contains(Environment.NewLine + "<i> - ", StringComparison.Ordinal)))
            {
                text = text.Remove(3, 2);
            }

            if (text != null && !text.Contains(Environment.NewLine, StringComparison.Ordinal) && (oldText.Contains(':') && !text.Contains(':') || oldText.Contains('[') && !text.Contains('[') || oldText.Contains('(') && !text.Contains('(') || oldText.Contains('{') && !text.Contains('{')) && (oldText.Contains(Environment.NewLine + "- ", StringComparison.Ordinal) || oldText.Contains(Environment.NewLine + " - ", StringComparison.Ordinal) || oldText.Contains(Environment.NewLine + "<i>- ", StringComparison.Ordinal) || oldText.Contains(Environment.NewLine + "<i> - ", StringComparison.Ordinal)))
            {
                text = text.TrimStart().TrimStart('-').TrimStart();
            }
        ...
        public string RemoveInterjections(string text)
        {
            ...
            if (this._interjectionList == null)
            {
                this._interjectionList = new List<string>();
                foreach (string s in arr)
                {
                    if (s.Length > 0)
                    {
                        if (!this._interjectionList.Contains(s))
                        {
                            this._interjectionList.Add(s);
                        }

                        string lower = s.ToLower();
                        if (!this._interjectionList.Contains(lower))
                        {
                            this._interjectionList.Add(lower);
                        }

                        string upper = s.ToUpper();
                        if (!this._interjectionList.Contains(upper))
                        {
                            this._interjectionList.Add(upper);
                        }

                        string pascalCasing = char.ToUpper(s[0]) + s.Substring(1);
                        if (!this._interjectionList.Contains(pascalCasing))
                        {
                            this._interjectionList.Add(pascalCasing);
                        }
                    }
                }

                this._interjectionList.Sort(CompareLength);
            }
            ...
            {
                doRepeat = false;
                foreach (string s in this._interjectionList)
                {
                    if (text.Contains(s))
                    {
            ...
            {
                if ((oldText.StartsWith('-') || oldText.StartsWith("<i>-", StringComparison.Ordinal)) && (oldText.Contains("." + Environment.NewLine) || oldText.Contains(".</i>" + Environment.NewLine) || oldText.Contains("!" + Environment.NewLine) || oldText.Contains("!</i>" + Environment.NewLine) || oldText.Contains("?" + Environment.NewLine) || oldText.Contains("?</i>" + Environment.NewLine)))
                {
                ...
                else if ((oldText.Contains(Environment.NewLine + "-") || oldText.Contains(Environment.NewLine + "<i>-")) && (oldText.Contains("." + Environment.NewLine) || oldText.Contains(".</i>" + Environment.NewLine) || oldText.Contains("!" + Environment.NewLine) || oldText.Contains("!</i>" + Environment.NewLine) || oldText.Contains("?" + Environment.NewLine) || oldText.Contains("?</i>" + Environment.NewLine)))
                {
                    if (text.StartsWith("<i>-"))
                    {
                        text = "<i>" + text.Remove(0, 4).TrimStart();
        ...
        public string RemoveHearImpairedTags(string text)
        {
            ...
            if (this.Settings.RemoveTextBetweenSquares)
            {
                text = RemoveTextBetweenTags("[", "]:", text);
                text = RemoveTextBetweenTags("[", "]", text);
            }

            if (this.Settings.RemoveTextBetweenBrackets)
            {
                text = RemoveTextBetweenTags("{", "}:", text);
                text = RemoveTextBetweenTags("{", "}", text);
            }

            if (this.Settings.RemoveTextBetweenQuestionMarks)
            {
                text = RemoveTextBetweenTags("?", "?:", text);
                text = RemoveTextBetweenTags("?", "?", text);
            }

            if (this.Settings.RemoveTextBetweenParentheses)
            {
                text = RemoveTextBetweenTags("(", "):", text);
                text = RemoveTextBetweenTags("(", ")", text);
            }

            if (this.Settings.RemoveTextBetweenCustomTags && this.Settings.CustomStart.Length > 0 && this.Settings.CustomEnd.Length > 0)
            {
                text = RemoveTextBetweenTags(this.Settings.CustomStart, this.Settings.CustomEnd, text);
            }
        ...
        public bool HasHearImpariedTagsAtStart(string text)
        {
            ...
            {
                return this.StartAndEndsWithHearImpariedTags(text);
            }
        ...
        public string RemoveLineIfAllUppercase(string text)
        {
            ...
            foreach (string line in lines)
            {
                string lineNoHtml = HtmlUtil.RemoveHtmlTags(line, true);
        ...
        private static string RemoveTextBetweenTags(string startTag, string endTag, string text)
        {
            ...
            while (start >= 0 && end > start)
            {
                ...
                if (start > 3 && start < text.Length - 1 && text.Substring(0, start + 1).EndsWith(" :", StringComparison.Ordinal) && ".!?".Contains(text[start - 2]))
                {
        ...
        private bool IsNotInsideBrackets(string text, int colonIdx)
        {
            ...
            int bIdx = text.IndexOfAny(new[] { '(', '[' });

            if (bIdx >= 0 && bIdx < colonIdx)
            {
                char closeType = text[bIdx] == '(' ? ')' : ']';
                int nIdx = text.IndexOf(closeType, bIdx + 1);
                if (nIdx > colonIdx)
                {
                    return false;
                }
        ...
        private bool IsHIDescription(string text)
        {
            ...
            if (text == "cackles" || text == "cheers" || text == "clears throat" || text == "chitters" || text == "chuckles" || text == "exclaims" || text == "exhales" || text == "gasps" || text == "grunts" || text == "groans" || text == "growls" || text == "explosion" || text == "laughs" || text == "noise" || text.EndsWith("on tv", StringComparison.Ordinal) || text.StartsWith("engine ", StringComparison.Ordinal) || text == "roars" || text == "scoff" || text == "screeches" || text.EndsWith("shatters", StringComparison.Ordinal) || text == "shouts" || text == "shrieks" || text == "sigh" || text == "sighs" || text == "snores" || text == "stutters" || text == "thuds" || text == "trumpets" || text == "whispers" || text == "whisper" || text == "whistles" || text.EndsWith("ing", StringComparison.Ordinal))
            {
                return true;
            }
        ...
        private string RemoveStartEndTags(string text)
        {
            string newText = text;
            string s = text;
            if (s.StartsWith('[') && s.IndexOf(']') > 0 && this.Settings.RemoveTextBetweenSquares)
            {
                newText = s.Remove(0, s.IndexOf(']') + 1);
            }
            else if (s.StartsWith('{') && s.IndexOf('}') > 0 && this.Settings.RemoveTextBetweenBrackets)
            {
                newText = s.Remove(0, s.IndexOf('}') + 1);
            }
            else if (s.StartsWith('?') && s.IndexOf('?', 1) > 0 && this.Settings.RemoveTextBetweenQuestionMarks)
            {
                newText = s.Remove(0, s.IndexOf('?', 1) + 1);
            }
            else if (s.StartsWith('(') && s.IndexOf(')') > 0 && this.Settings.RemoveTextBetweenParentheses)
            {
                newText = s.Remove(0, s.IndexOf(')') + 1);
            }
            else if (s.StartsWith('[') && s.IndexOf("]:", StringComparison.Ordinal) > 0 && this.Settings.RemoveTextBetweenSquares)
            {
                newText = s.Remove(0, s.IndexOf("]:", StringComparison.Ordinal) + 2);
            }
            else if (s.StartsWith('{') && s.IndexOf("}:", StringComparison.Ordinal) > 0 && this.Settings.RemoveTextBetweenBrackets)
            {
                newText = s.Remove(0, s.IndexOf("}:", StringComparison.Ordinal) + 2);
            }
            else if (s.StartsWith('?') && s.IndexOf("?:", 1, StringComparison.Ordinal) > 0 && this.Settings.RemoveTextBetweenQuestionMarks)
            {
                newText = s.Remove(0, s.IndexOf("?:", StringComparison.Ordinal) + 2);
            }
            else if (s.StartsWith('(') && s.IndexOf("):", StringComparison.Ordinal) > 0 && this.Settings.RemoveTextBetweenParentheses)
            {
                newText = s.Remove(0, s.IndexOf("):", StringComparison.Ordinal) + 2);
            }
            else if (this.Settings.RemoveTextBetweenCustomTags && s.Length > 0 && this.Settings.CustomEnd.Length > 0 && this.Settings.CustomStart.Length > 0 && s.StartsWith(this.Settings.CustomStart) && s.LastIndexOf(this.Settings.CustomEnd, StringComparison.Ordinal) > 0)
            {
                newText = s.Remove(0, s.LastIndexOf(this.Settings.CustomEnd, StringComparison.Ordinal) + this.Settings.CustomEnd.Length);
            }
