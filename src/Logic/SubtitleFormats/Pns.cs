...
namespace Nikse.SubtitleEdit.Logic.SubtitleFormats
{
    ...
    public class Pns : SubtitleFormat
    {
            ...
            if (!string.IsNullOrEmpty(fileName) && File.Exists(fileName) && fileName.EndsWith(".pns", StringComparison.OrdinalIgnoreCase))
            {
                try
                {
                    ...
                    if (fi.Length > 100 && fi.Length < 1024000)
                    {
                        ...
                        if (buffer[00] != 0 && buffer[01] == 0 && buffer[02] == 0 && buffer[03] == 0 && buffer[04] == 0 && buffer[05] == 0 && buffer[06] == 0 && buffer[07] == 0 && buffer[08] == 0 && buffer[09] == 0 && buffer[10] == 0 && buffer[11] == 0 && buffer[12] == 0 && buffer[13] == 0 && buffer[14] == 0 && buffer[15] == 0 && buffer[16] == 0 && buffer[17] == 0 && buffer[18] == 0 && buffer[19] == 0)
                        {
                            Subtitle sub = new Subtitle();
                            this.LoadSubtitle(sub, null, fileName);
        ...
        public override void LoadSubtitle(Subtitle subtitle, List<string> lines, string fileName)
        {
            ...
            {
                Paragraph p = this.GetParagraph(ref index, buffer);
                if (p != null)
                {
        ...
        private Paragraph GetParagraph(ref int index, byte[] buffer)
        {
            while (index < buffer.Length - 20)
            {
                ...
                if (buffer.Length > index + 20 && buffer[index + 00] == 0 && buffer[index + 01] == 0 &&

                    // buffer[index + 02] == 0 && // start min
                    // buffer[index + 03] == 0 &&
                    buffer[index + 04] == 0 && buffer[index + 05] == 0 &&
                    ...
                    // buffer[index + 09] == 0 &&
                    buffer[index + 10] == 0 && buffer[index + 11] == 0 &&
                    ...
                    buffer[index + 15] == 0)
                {
                    ...
                    if (buffer.Length > index + 15 + textLength)
                    {
                        ...
                        index += 15 + textLength;
                        Paragraph p = new Paragraph(text, startSeconds * 1000 + FramesToMillisecondsMax999(startFrame), endSeconds * 1000 + FramesToMillisecondsMax999(endFrame));
                        return p;
                    }
