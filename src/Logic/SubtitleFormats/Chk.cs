...
namespace Nikse.SubtitleEdit.Logic.SubtitleFormats
{
    ...
    public class Chk : SubtitleFormat
    {
        ...
        public override void LoadSubtitle(Subtitle subtitle, List<string> lines, string fileName)
        {
            ...
            {
                Paragraph p = this.ReadParagraph(buffer, index);
                if (p != null)
                {
        ...
        private Paragraph ReadParagraph(byte[] buffer, int index)
        {
            if (buffer[index] == 1 && this._timeCodeQueue.Count > 0)
            {
                ...
                for (int i = 0; i <= textLength; i++)
                {
                    ...
                    {
                        sb.Append(this.GetText(buffer, start, index + i + 13));
                        start = index + 13 + i + 3;
                        skipCount = 2;
                        sb.AppendLine();
                    }
                    ...
                    if (i == textLength)
                    {
                        sb.Append(this.GetText(buffer, start, index + i + 13 + 1));
                    }
                }

                Paragraph p;
                if (this._timeCodeQueue.Count > 0)
                {
                    p = this._timeCodeQueue.Dequeue();
                }
                else
                {
                    p = new Paragraph();
            ...
            if (buffer[index] == 0x0a && this._timeCodeQueue.Count > 0)
            {
                // ?
            }
            else if (buffer[index] == 0x09 && this._timeCodeQueue.Count > 0)
            {
                // ?
            }
            else
            {
                // time codes
                this._timeCodeQueue = new Queue<Paragraph>();
                for (int i = 0; i < 15; i++)
                {
                    ...
                    Paragraph p = new Paragraph(string.Empty, FramesToMilliseconds(totalFrameNumber), FramesToMilliseconds(totalFrameNumber + durationInFrames));
                    this._timeCodeQueue.Enqueue(p);
                }
            }
        ...
        private string GetText(byte[] buffer, int start, int end)
        {
            ...
            if (buffer[start] == 0x1f && buffer[start + 1] == 0x57 && buffer[start + 2] == 0x31 && buffer[start + 3] == 0x36)
            {
                // W16
                if (end - start > 4)
                {
            ...
            else
            {
                if (end - start > 0)
                {
                    text = this._codePage.GetString(buffer, start, end - start);
                }
            }
