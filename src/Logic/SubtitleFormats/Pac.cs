...
namespace Nikse.SubtitleEdit.Logic.SubtitleFormats
{
    ...
    public class Pac : SubtitleFormat
    {
        ...
        /// </summary>
        private static readonly List<int> LatinCodes = new List<int> { 0xe041, // Ã
                                                                       0xe04e, // Ñ
                                                                       0xe04f, // Õ
                                                                       0xe061, // ã
                                                                       0xe06e, // ñ
        ...
        public void Save(string fileName, Subtitle subtitle)
        {
            using (FileStream fs = new FileStream(fileName, FileMode.Create, FileAccess.Write))
            {
                ...
                {
                    this.WriteParagraph(fs, p, number, number + 1 == subtitle.Paragraphs.Count);
                    number++;
                }
                ...
                try
                {
                    ...
                    if (fi.Length > 100 && fi.Length < 1024000)
                    {
                        ...
                        if (buffer[00] == 1 && // These bytes seems to be PAC files... TODO: Verify!
                            buffer[01] == 0 && buffer[02] == 0 && buffer[03] == 0 && buffer[04] == 0 && buffer[05] == 0 && buffer[06] == 0 && buffer[07] == 0 && buffer[08] == 0 && buffer[09] == 0 && buffer[10] == 0 && buffer[11] == 0 && buffer[12] == 0 && buffer[13] == 0 && buffer[14] == 0 && buffer[15] == 0 && buffer[16] == 0 && buffer[17] == 0 && buffer[18] == 0 && buffer[19] == 0 && buffer[20] == 0 &&

                            // buffer[21] < 10 && // start from number
                            // buffer[22] == 0 &&
                            buffer[23] == 0x60 && fileName.EndsWith(".pac", StringComparison.OrdinalIgnoreCase))
                        {
                            return true;
                        }
                    }
        ...
        public override void LoadSubtitle(Subtitle subtitle, List<string> lines, string fileName)
        {
            ...
            {
                Paragraph p = this.GetPacParagraph(ref index, buffer);
                if (p != null)
                {
        ...
        private static string MakePacItalicsAndRemoveOtherTags(string text)
        {
            ...
            string[] parts = text.SplitToLines();
        ...
        private static byte[] GetLatinBytes(Encoding encoding, string text, byte alignment)
        {
            ...
            while (i < text.Length)
            {
                ...
                else
                {
                    ...
                    {
                        byte[] values = encoding.GetBytes(letter);
        ...
        private static byte[] GetBytesViaLists(string text, List<string> letters, List<int> codes, byte alignment)
        {
            ...
            while (i < text.Length)
            {
                ...
                else
                {
                    ...
                    {
                        byte[] values = Encoding.Default.GetBytes(letter);
                        for (int k = 0; k < values.Length; k++)
                        {
                            byte v = values[k];
                            if (k > 0)
        ...
        private static byte[] GetChineseBig5Bytes(string text, byte alignment)
        {
            ...
            foreach (string line in text.SplitToLines())
            {
                ...
                if (OnlyAnsi(line))
                {
                    foreach (byte b in GetLatinBytes(GetEncoding(CodePageLatin), line, alignment))
                    {
                        result.Add(b);
                    }
                }
                else
                {
                    ...
                    foreach (byte b in Encoding.GetEncoding(950).GetBytes(line))
                    {
                        result.Add(b);
                    }
                }
        ...
        private void WriteParagraph(FileStream fs, Paragraph p, int number, bool isLast)
        {
            ...
            if (this._codePage == CodePageArabic)
            {
            ...
            else if (this._codePage == CodePageHebrew)
            {
            ...
            else if (this._codePage == CodePageLatin)
            {
                textBuffer = GetLatinBytes(encoding, text, alignment);
            }
            else if (this._codePage == CodePageCyrillic)
            {
                textBuffer = GetCyrillicBytes(text, alignment);
            }
            else if (this._codePage == CodePageChinese)
            {
            ...
            // write text length
            ushort length = (ushort)(textBuffer.Length + 4);
        ...
        private Paragraph GetPacParagraph(ref int index, byte[] buffer)
        {
            ...
            if (this._codePage == -1)
            {
                this.GetCodePage(buffer, index, endDelimiter);
            }
            ...
            while (index < buffer.Length && index <= maxIndex)
            {
                // buffer[index] != endDelimiter)
                ...
                {
                    sb.Append(GetLatinString(GetEncoding(this._codePage), buffer, ref index));
                }
                else if (this._codePage == CodePageArabic)
                {
                ...
                {
                    sb.Append(GetEncoding(this._codePage).GetString(buffer, index, 1));
                }
        ...
        private int AutoDetectEncoding()
        {
            try
            {
                byte[] buffer = FileUtil.ReadAllBytesShared(this._fileName);
                int index = 0;
                int count = 0;
                this._codePage = CodePageLatin;
                while (index < buffer.Length)
                {
                    int start = index;
                    Paragraph p = this.GetPacParagraph(ref index, buffer);
                    ...
                    {
                        this._codePage = CodePageLatin;
                        StringBuilder sb = new StringBuilder("ABCDEFGHIJKLMNOPPQRSTUVWXYZÆØÅÄÖÜabcdefghijklmnopqrstuvwxyzæøäåü(1234567890, .!?-\r\n'\"):;&");
                        foreach (string s in LatinLetters)
                        {
                            sb.Append(s);
                        }

                        string codePageLetters = sb.ToString();
                        bool allOk = true;
                        ...
                        this._codePage = CodePageGreek;
                        index = start;
                        p = this.GetPacParagraph(ref index, buffer);
                        ...
                        this._codePage = CodePageCyrillic;
                        index = start;
                        p = this.GetPacParagraph(ref index, buffer);
                        sb = new StringBuilder("(1234567890, .!?-\r\n'\"):;&");
