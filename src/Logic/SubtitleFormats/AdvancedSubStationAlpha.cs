    ...
    public class AdvancedSubStationAlpha : SubtitleFormat
    {
        ...
        public static string DefaultStyle
        {
            get
            {
                ...
                return "Style: Default," + Configuration.Settings.SubtitleSettings.SsaFontName + "," + ((int)Configuration.Settings.SubtitleSettings.SsaFontSize) + "," + GetSsaColorString(Color.FromArgb(Configuration.Settings.SubtitleSettings.SsaFontColorArgb)) + "," + "&H0300FFFF,&H00000000,&H02000000,0,0,0,0,100,100,0,0," + borderStyle + "," + Configuration.Settings.SubtitleSettings.SsaOutline.ToString(CultureInfo.InvariantCulture) + "," + Configuration.Settings.SubtitleSettings.SsaShadow.ToString(CultureInfo.InvariantCulture) + ",2,10,10,10,1";
            }
        }
        ...
        public static string CheckForErrors(string header)
        {
            ...
            foreach (string line in header.SplitToLines())
            {
                ...
                if (s.StartsWith("format:", StringComparison.Ordinal))
                {
                    if (line.Length > 10)
                    {
                        string[] format = line.Substring(8).ToLower().Split(',');
            ...
            foreach (string line in lines)
            {
                ...
                else if (eventsStarted)
                {
                    ...
                    else if (!string.IsNullOrEmpty(s))
                    {
                        ...
                        try
                        {
                            Paragraph p = new Paragraph { StartTime = GetTimeCodeFromString(start), EndTime = GetTimeCodeFromString(end), Text = GetFormattedText(text) };
        ...
        private static void LoadStylesFromSubstationAlpha(Subtitle subtitle, string title, string header, string headerNoStyles, StringBuilder sb)
        {
            try
            {
                ...
                foreach (string styleName in GetStylesFromHeader(subtitle.Header))
                {
                    try
                    {
                        SsaStyle ssaStyle = GetSsaStyle(styleName, subtitle.Header);
                        if (ssaStyle != null)
                        {
                            string bold = "0";
                            if (ssaStyle.Bold)
        ...
        private static string RemoveTag(string s, string tag)
        {
            ...
            if (indexOfTag > 0)
            {
                int endIndex1 = s.IndexOf('\\', indexOfTag + 1);
                int endIndex2 = s.IndexOf('}', indexOfTag + 1);
